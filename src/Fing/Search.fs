...
let rec roundRobin : list<'a> -> list<'a * list<'a>> = 
    function 
    | [] -> []
    | x :: xs -> 
        (x, xs) :: List.map (fun (y, ys) -> (y, x :: ys)) (roundRobin xs)
...
let rec factorialOrder = 
    function 
    | [] -> [ [] ]
    | l when List.length l > 5 -> [ l ]
    | l -> 
        [ for x, xs in roundRobin l do
              for ys in factorialOrder xs -> x :: ys ]

let argOrder ts = 
    if List.length ts = 2 then [ ts ]
    else 
        let args, ret = Seq.butLast ts
        List.map (fun args -> args @ [ ret ]) (factorialOrder (List.ofSeq args))

let rec variants : Typ -> Typ list = 
    function 
    | Arrow ts -> 
        ts
        |> List.map variants
        |> argOrder
        |> List.collect listSequence
        |> List.map Arrow
    | Tuple ts -> 
        ts
        |> List.map variants
    ...
    | Var v -> [ Var v ]
    | Id id -> [ Id id ]
    | NamedArg(name, t, isOpt) -> 
        List.map (fun t -> NamedArg(name, t, isOpt)) (variants t)
