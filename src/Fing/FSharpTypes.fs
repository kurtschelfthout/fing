...
let rec cvt (e : FSharpType) = 
    if e.IsTupleType then 
        Tuple(e.GenericArguments
              |> Seq.map cvt
              |> List.ofSeq)
    elif e.IsFunctionType then 
        Arrow(e.GenericArguments
              |> Seq.map cvt
              |> List.ofSeq)
    elif e.IsGenericParameter then cvtParam e.GenericParameter // TODO: cvtParam is very much not done
        ...
        match e.TypeDefinition |> canonicalType, 
              e.GenericArguments
              |> Seq.map cvt
              |> List.ofSeq with
        | t, [] -> t
...
and cvtParam (param : FSharpGenericParameter) = 
    if Seq.isEmpty param.Constraints then Var(Normal param.Name)
    else 
        match param.Constraints 
              |> Seq.tryFind 
                     (fun c -> 
                     c.IsDefaultsToConstraint 
                     && c.DefaultsToConstraintData.DefaultsToTarget.HasTypeDefinition) with
        | Some def -> 
            def.DefaultsToConstraintData.DefaultsToTarget.TypeDefinition 
            |> canonicalType
...
and canonicalType (e : FSharpEntity) = 
    if e.IsFSharpAbbreviation then cvt e.AbbreviatedType
    else 
...
and whenify (param : FSharpGenericParameter) 
    ...
    if con.IsSupportsNullConstraint then Null(Normal param.Name)
    elif con.IsReferenceTypeConstraint then NotStruct(Normal param.Name)
    elif con.IsNonNullableValueTypeConstraint then Struct(Normal param.Name)
        ...
        Delegate
            (Normal param.Name, 
             cvt con.DelegateConstraintData.DelegateTupledArgumentType, 
             cvt con.DelegateConstraintData.DelegateReturnType)
    elif con.IsCoercesToConstraint then 
        Subtype(Normal param.Name, cvt con.CoercesToTarget)
    elif con.IsMemberConstraint then 
        Sig
            (Structural param.Name, Id con.MemberConstraintData.MemberName, 
             
             Arrow
                 (List.ofSeq 
                      (Seq.map cvt con.MemberConstraintData.MemberArgumentTypes) 
                  @ [ cvt con.MemberConstraintData.MemberReturnType ]), Function)
    else failwith "basically this style of data structure just sucks"
