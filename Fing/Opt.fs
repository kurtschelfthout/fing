...
type Argument = 
    | Bare of string
    | Parameter of string * option<string>
...
let mapGet key map value = 
    if Map.containsKey key map then map.[key]
    else value
...
let parse (args : string []) = 
    ...
    and param p = 
        function 
        | [] -> 
            if isSingle p then [ fromSingle p ]
            else [ Parameter(p, None) ]
        | (a :: args) when isParam a -> 
            (if isSingle p then fromSingle p
             else Parameter(p, None)) :: param (unParam a) args
